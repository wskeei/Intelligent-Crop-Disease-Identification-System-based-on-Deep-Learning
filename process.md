...
#### 3.1 基础架构 (P0 - 必须完成)
* [ ] **脚手架搭建**: 初始化 Vue3 + TypeScript + Vite 项目
* [ ] **依赖安装**: Element Plus + Pinia + Vue Router + Axios + ECharts
* [ ] **基础布局**: 实现 AppLayout (侧边栏 + 顶栏 + 内容区)
* [ ] **路由配置**: 配置 6 个主要页面路由
* [ ] **API 封装**: Axios 实例配置，请求/响应拦截器

#### 3.2 核心功能页面 (P0 - 必须完成)
* [ ] **智能识别页**: 图片上传 + 拖拽上传 + 预览 + 识别结果展示
* [ ] **识别结果组件**: 病害名称 + 置信度进度条 + Top-3 结果
* [ ] **历史记录页**: 列表/卡片视图 + 筛选 + 分页 + 删除

#### 3.3 数据可视化 (P1 - 重要功能)
* [ ] **工作台页面**: 统计卡片 + 快捷入口 + 最近记录
* [ ] **数据分析页**: ECharts 图表大屏 (饼图/折线图/柱状图/热力图)
* [ ] **图表组件封装**: 通用 ECharts 组件

#### 3.4 扩展功能 (P2 - 增强体验)
* [ ] **病害知识库**: 作物分类导航 + 病害卡片列表 + 详情页
